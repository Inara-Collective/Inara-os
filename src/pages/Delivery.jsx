...
export default function Delivery() {
  ...
  const upd = async (id, field, value) => { const u = await updateTask(id,{[field]:value}); setTasks(p=>p.map(t=>t.id===id?u:t)) }
  ...
  const filtered = filterClient ? tasks.filter(t=>t.client_id===filterClient) : tasks
    ...
    <div>
      <div className="topbar">
        ...
        <div className="topbar-actions">
          <select className="form-select" style={{width:200,padding:'.4rem .75rem',fontSize:'.78rem'}} value={filterClient} onChange={e=>setFilterClient(e.target.value)}>
